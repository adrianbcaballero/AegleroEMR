...
> **Auto-generated** by the Aeglero compliance engine on 2026-07-01T00:39:08+00:00. Do not hand-edit; regenerate from `status.json`.
...
- **SPRS score:** 97 / 110 (−13) — _partial catalog (10 of 110 controls seeded); score is conservative (unassessed = not implemented)_
- **Automated evidence coverage:** 80.0% (8 of 10 applicable controls)
- **Status breakdown:** {'met': 7, 'not-collected': 2, 'partial': 1}
...
**Evidence.**
- `backend/services/audit_logger.py:29` — log_access() creates and stores an AuditLog row per event.

**Provenance.** collected by `audit_chain` at 2026-07-01T00:39:08+00:00 · evidence SHA-256 `f44ccc756422e990…`
...
**Evidence.**
- `backend/services/audit_logger.py:9` — user_id is part of the audit record content and hash input.

**Provenance.** collected by `audit_chain` at 2026-07-01T00:39:08+00:00 · evidence SHA-256 `3041c5b18177bd66…`
...
**Evidence.**
- `backend/services/audit_logger.py:9` — _compute_hash() = SHA-256(content || prev_hash) builds the chain.
- `backend/services/audit_logger.py:9` — Each entry references the previous entry's hash (per-tenant chain).
- `backend/routes/audit.py:246` — GET /api/audit/verify walks the chain and reports tampering.

**Provenance.** collected by `audit_chain` at 2026-07-01T00:39:08+00:00 · evidence SHA-256 `7e1ab282570bf134…`
...
**Evidence.**
- `backend/auth_middleware.py:47` — require_auth() authenticates the session before the handler runs.

**Provenance.** collected by `access_control` at 2026-07-01T00:39:08+00:00 · evidence SHA-256 `c052f0e419b984d4…`
...
**Evidence.**
- `backend/auth_middleware.py:71` — require_auth(permission=...) enforces per-route permission.
- `backend/routes/patients.py:148` — _apply_rbac() limits patient rows to the caller's care teams.

**Provenance.** collected by `access_control` at 2026-07-01T00:39:08+00:00 · evidence SHA-256 `e03f1d4c6bf4772c…`
...
**Evidence.**
- `backend/routes/mfa.py:4` — pyotp TOTP setup/verify implements the second factor.
- `backend/routes/auth.py:71` — Login enforces TOTP when tenant.mfa_required is enabled.

**Provenance.** collected by `access_control` at 2026-07-01T00:39:08+00:00 · evidence SHA-256 `3f070c86830d53df…`
...
**Evidence.**
- `.github/workflows/ci.yml` — 5 scanners configured: Bandit (SAST), pip-audit (Python deps), Trivy (containers/IaC/deps), Checkov (IaC compliance), pnpm audit (JS deps).
- `.github/workflows/ci.yml` — A HIGH/CRITICAL finding fails the build (exit-code 1), blocking merge until the flaw is corrected.

**Provenance.** collected by `flaw_remediation` at 2026-07-01T00:39:08+00:00 · evidence SHA-256 `3f153b6b1e8f5801…`
...
**Evidence.**
- `.github/workflows/ci.yml` — Scan frequency defined by triggers: push, pull_request.
- `.github/workflows/ci.yml` — POA&M: add a scheduled (cron) scan to satisfy 3.11.2[d] for newly identified vulnerabilities.

**Provenance.** collected by `flaw_remediation` at 2026-07-01T00:39:08+00:00 · evidence SHA-256 `db60d2bac5b02da8…`
